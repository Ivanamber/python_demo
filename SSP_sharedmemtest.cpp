...
void fl_SSP_InitDB()
{
    ...
    printf("key:%d\r\n", key);
//    key_t key = 0x01020001;
    shm_id = shmget(key, SHM_DB_Size,IPC_CREAT|0666);        //creat
    if(shm_id == -1)
    {
        printf("init create share memory fail!\r\n");
        exit(1);
    }
}
...
int main()
{
    ...
    printf("get date %s\r\n",DataBase.SignalArry[2].name);
    printf("get date %d\r\n",DataBase.SignalArry[2].enable);
    printf("get date %f\r\n",DataBase.SignalArry[2].value_send);
    printf("get date %f\r\n",DataBase.SignalArry[2].value_rcv);
    printf("get date %f\r\n",DataBase.SignalArry[2].errrange);
    //fg_SSPApp_RmDB();
    printf("3\r\n");
    sleep(1);
    printf("4\r\n");
